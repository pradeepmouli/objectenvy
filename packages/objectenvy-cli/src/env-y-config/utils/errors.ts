...
/**
 * Create error with format validation details
 */
export function createInvalidFormatError(filePath: string): ConversionError {
  return new ConversionError(
    ErrorCategory.INPUT,
    EXIT_CODES.INVALID_FORMAT,
    `Unsupported input format: ${filePath}`,
    'The file extension or format is not supported.',
    'Supported formats: .ts (Zod/TypeScript), .json (JSON Schema/JSON)'
  );
}
...
export function createValidationError(message: string, detail?: string): ConversionError {
  return new ConversionError(
    ...
    message,
    detail,
    'Check the schema definition and ensure all required fields are valid'
  );
}
